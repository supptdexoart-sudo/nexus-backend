...
export const TrapCard: React.FC<TrapCardProps> = ({ event, onClose, onPlayerDamage, playerClass, onClaimLoot }) => {
    ...
    return (
        <div className="space-y-4">
            {/* --- STATUS HEADER --- */}
            <div className={`p-4 rounded-xl border-2 flex items-center justify-between transition-colors ${trapStatus === 'active' ? 'bg-orange-950/30 border-orange-500/50' :
                    trapStatus === 'success' || trapStatus === 'claimed' ? 'bg-green-950/30 border-green-500' :
                        'bg-red-950/30 border-red-600'
                }`}>
                <div className="flex items-center gap-3">
                    <div className={`p-3 rounded-full border ${trapStatus === 'active' ? 'bg-orange-500/10 border-orange-500 text-orange-500' :
                    ...
                    <div>
                        <h3 className={`text-sm font-black uppercase tracking-widest ${trapStatus === 'active' ? 'text-orange-500' :
            ...
            {trapStatus === 'success' && (
                <div className="animate-in zoom-in duration-300 space-y-4">
                    ...
                    <button
                        onClick={handleClaim}
                        className="w-full py-4 bg-green-600 active:bg-green-500 text-white font-black uppercase text-xs tracking-[0.2em] rounded-xl shadow-[0_0_20px_rgba(34,197,94,0.3)] flex items-center justify-center gap-2 transition-all active:scale-95"
                    >
                        {config.loot && config.loot.length > 0 ? (
                    ...
                    {canDisarm ? (
                        <div className="animate-in fade-in slide-in-from-bottom-2">
                            ...
                            <button
                                ...
                                className="w-full py-4 bg-green-600 active:bg-green-500 text-white font-black uppercase text-xs tracking-[0.2em] rounded-xl flex items-center justify-center gap-3 transition-all active:scale-95 shadow-[0_0_20px_rgba(34,197,94,0.4)] animate-pulse"
                            >
                        ...
                        <>
                            ...
                            <div className="flex gap-2 h-20">
                                <button
                                    ...
                                    disabled={isRolling}
                                    className="flex-1 bg-zinc-800 active:bg-zinc-700 text-white font-black uppercase text-[10px] tracking-[0.1em] rounded-xl flex flex-col items-center justify-center gap-2 border border-zinc-600 active:scale-95 transition-all group"
                                >
                                    <Keyboard className="w-6 h-6 text-zinc-400 group-active:text-white transition-colors" />
                                ...
                                <button
                                    ...
                                    disabled={isRolling}
                                    className="w-24 bg-orange-600 active:bg-orange-500 text-white rounded-xl shadow-[0_0_15px_rgba(249,115,22,0.4)] active:scale-95 transition-all flex flex-col items-center justify-center border border-orange-400"
                                >
