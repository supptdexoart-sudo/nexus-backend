...
app.post('/api/inventory/:email', (req, res) => {
    ...
    // Check if item exists, if so update it, else push
...
app.delete('/api/inventory/:email/:cardId', (req, res) => {
    ...
    if (user.inventory.length < initialLength) {
        console.log(`Deleted item ${req.params.cardId} from ${user.email}`);
...
// NEW LOGIC: SEND GIFT (Remove from Sender ONLY)
app.post('/api/inventory/send-gift', (req, res) => {
    const { fromEmail, cardId } = req.body;
    console.log(`GIFT REQUEST from: ${fromEmail}, item: ${cardId}`); 
    ...
    // REMOVE from Sender
    const [removedItem] = sender.inventory.splice(itemIndex, 1);
    
    // Return the item data so the frontend can send it via Chat
    console.log(`GIFT SUCCESS: Item ${targetId} removed from ${fromEmail}`);
...
app.post('/api/gemini/interpret-code', (req, res) => {
    res.json({
        id: req.body.code,
        title: "Mystery Item",
        description: "Generated by backend mock.",
        type: "PŘEDMĚT",
        rarity: "Common",
        stats: []
    });
